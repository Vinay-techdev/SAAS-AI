...
const WriteArticle = () => {
  ...
  const [selectedLength, setSelectedLenght] = useState(articleLenght[0])
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {
            articleLenght.map((item, index) =>(
              <span onClick={()=> setSelectedLenght(item)} key={index} className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedLength.text === item.text ? 'bg-blue-50 text-blue-700' : 'text-gray-500 border-gray-300'}`}>
                {item.text}
              </span>
            ))
          }
